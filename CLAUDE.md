...
This project is indexed by GitNexus as **CoXAgent** (4875 symbols, 12743 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- Clippy baseline measurement requires a delta against the tree you started from, not an absolute target — run the count command once with changes, stash, re-run for baseline, then diff the sorted error lists. Workspace clippy hides downstream crates: if application aborts the check, presentation/app are never linted. Baseline against `origin/main` in a detached worktree, not the local `main` branch, to avoid phantom improvements from upstream fixes. Before shipping, confirm `cargo fmt --all --check` passes and main is clean, so you fix only the branch's own drift.
- Index staleness when concurrent processes touch files in a shared working tree is a real git edge case. Teams using multi-session or multi-container workflows should document the `git update-index --refresh` fix in CLAUDE.md under troubleshooting, so the step is visible, discoverable, and applied consistently.
