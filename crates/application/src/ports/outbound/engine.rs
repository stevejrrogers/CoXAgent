...
pub enum SandboxStatus {
    ...
    /// to apply the profile on every attempt — so the agent never ran
    /// at all (COX-B016). Distinct from [`Self::Unavailable`], where the agent
    /// did its work, just unconfined: here there is no work and no output, and
    /// the non-zero exit belongs to the sandbox rather than the agent.
    Refused(&'static str),
}
