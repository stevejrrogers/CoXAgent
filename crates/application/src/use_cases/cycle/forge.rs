...
impl<S: StateStorePort, E: AgentEnginePort> RunCycleUseCase<S, E> {
    ...
    pub(super) async fn commit_for_ticket(&self, id: &TicketId, kind: &str) {
        ...
        if let Err(e) = git.checkout_branch(&self.work_dir, &branch).await {
            // Rejected-proposal debris or leftover agent residue in the tree
            // blocks even switching to the ticket's branch. Rather than blame
            // a `Fixed`/`Done` ticket forever (the repeated "branch failed"
            // spam — see the sweep doc), hand the tree to a DEV agent to
            // reconcile it intelligently: keep THIS ticket's legitimate work,
            // drop debris from past attacks a human already rejected (the
            // ticket journal names them), commit the survivor to `branch`, and
            // leave the tree branch-switchable. One bounded attempt; if it
            // still cannot ship, report and move on (no infinite loop).
