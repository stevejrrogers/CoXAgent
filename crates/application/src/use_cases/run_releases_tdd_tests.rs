...
mod tdd_tests {
    ...
    async fn ac1_creates_tag_at_exact_target() {
        ...
        let (store, git) = setup(
            ver,
            &milestone(ver, name, "Goal", true, false),
        ...
        assert!(
            released.contains(&name.to_string()),
    ...
    async fn ac1_creates_tag_when_version_exceeds_target() {
        ...
        assert!(
            !git.tags_created.lock().expect("lock").is_empty(),
    ...
    async fn ac2_notes_include_ticket_titles_not_just_ids() {
        ...
        assert!(
            chore.description().contains(deploy_title),
            "notes must include deploy title: {}",
            chore.description()
        );
    }
    ...
    async fn ac3_skips_existing_tag_and_logs_activity() {
        ...
        assert!(
            ae.is_some(),
            "activity must be logged with 'release already exists'"
        );
        assert_eq!(state.tickets.len(), 0, "no new ticket when tag exists");
    }
    ...
    async fn ac4_creates_chore_with_dependencies_and_fulfills_milestone() {
        ...
        assert!(state.milestones[0].fulfilled, "milestone must be fulfilled");

        let deps = chore.depends_on();
        assert!(
            deps.iter().any(|d| d.to_string() == "CXA-P1"),
            "chore depends on CXA-P1"
        );
        assert!(
            deps.iter().any(|d| d.to_string() == "CXA-P2"),
    ...
    async fn ac_handles_empty_deploy_history() {
        ...
        let _ = uk(store.clone(), git.clone())
            .execute()
            .await
            .expect("must handle empty history");
    ...
    async fn ac_releases_multiple_milestones_in_one_pass() {
        ...
        let released = uk(store.clone(), git.clone())
            .execute()
            .await
            .expect("pipeline releases all");

        assert!(
            released.contains(&"M1".to_string()),
            "first milestone released: {released:?}"
        );
        assert!(
            released.contains(&"M2".to_string()),
