...
fn catalog_is_discoverable_and_nonempty() {
    ...
    for e in &entries {
        assert!(
            !e.template.is_empty(),
            "catalog entry `{}` has an empty template",
            e.key
        );
        for p in &e.params {
            assert!(
...
fn every_paramized_entry_fills_its_tokens_with_long_values() {
    ...
    assert!(
        !entries.is_empty(),
        "catalog did not parse — parser out of sync with copy.js"
    );
    for e in &entries {
        assert!(
        ...
        let params: Vec<(&str, String)> = e
            .params
            .iter()
            .map(|p| (p.as_str(), LONG.clone()))
            .collect();
...
fn tooltip_surfaces_keep_full_text_in_title_and_aria_label() {
    ...
    let keys: Vec<&Entry> = entries
        .iter()
        .filter(|e| {
            (e.key.starts_with("empty.") && !e.params.is_empty()) || e.key.starts_with("error.")
        })
        .collect();
    assert!(
        keys.len() >= 2,
    ...
    for e in &keys {
        ...
        assert_eq!(
            title, text,
            "ellipsis `{}`: title must equal the full text",
            e.key
        );
        assert_eq!(
            aria, text,
            "ellipsis `{}`: aria-label must equal the full text",
            e.key
        );
        assert!(
...
fn button_labels_render_their_full_text_for_long_labels_too() {
    let entries = parse_catalog(COPY_JS);
    let buttons: Vec<&Entry> = entries
        .iter()
        .filter(|e| e.key.starts_with("action."))
        .collect();
    assert!(
        buttons.len() >= 2,
    ...
    for e in &buttons {
        ...
        // buttons are fixed labels; the empty-render assert is the contract
    }
}
...
fn copy_js_wires_the_three_inventory_renderers_with_accessible_attributes() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "copy.js is missing required renderer wiring: {needle:?}"
        );
    }
}
