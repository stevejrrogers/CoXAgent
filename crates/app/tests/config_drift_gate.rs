...
fn coverage_section_round_trips_exactly_on_disk() {
    ...
    assert_eq!(back.coverage.enabled, false, "enabled must survive the cycle unchanged");
...
fn prior_version_state_migrates_and_preserves_user_coverage() {
    ...
    assert_eq!(migrated.coverage.enabled, false, "user-set enabled must survive migration");
    assert_eq!(
