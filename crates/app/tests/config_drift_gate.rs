...
// Integration-test scaffolding operates on throwaway temp dirs where a failure
// means "the environment itself broke", not a recoverable code path; unwrap /
// expect are idiomatic here exactly as they are inside unit-test modules.
#![allow(clippy::unwrap_used, clippy::expect_used)]

use coxagent_application::config::{CONFIG_SCHEMA_VERSION, Config};
...
fn persist_and_read(value_in_memory: &Value) -> String {
    ...
    std::fs::write(&path, serde_json::to_string_pretty(value_in_memory).expect("serialize"))
...
fn coverage_section_round_trips_exactly_on_disk() {
    ...
    assert!(!back.coverage.enabled, "enabled must survive the cycle unchanged");
    assert_eq!(back.coverage.threshold, 7, "threshold must survive the cycle unchanged");
}
...
fn newer_persisted_schema_is_refused_at_load_not_defaulted() {
    ...
    assert!(
        outcome.is_err(),
        "a persisted schema newer than supported ({CONFIG_SCHEMA_VERSION}) must refuse to load, \
         not default away"
    );
}
...
fn threshold_change_reaches_the_next_pass_without_a_restart() {
    ...
    assert_eq!(
        next_pass_cfg.coverage.threshold,
        9,
...
fn prior_version_state_migrates_and_preserves_user_coverage() {
    ...
    assert!(!migrated.coverage.enabled, "user-set enabled must survive migration");
    assert_eq!(
