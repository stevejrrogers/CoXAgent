...
CXA-F022 is a burn-down ticket: it clears four open bugs that gate the F001 prompt-system feature from shipping. It does so by establishing a proof standard — an open gating bug is not burned down just because its ticket reaches `Verified`; it must also carry recorded QA evidence of a passing *root-cause* regression test in its own provenance (never a symptom mask or workaround probe). This page documents that standard, who enforces it (the TEST agent's verification path), and the four contracts (`g1`–`g4`) the fixes must preserve on the F001 prompt system.

It is for engineers fixing or re-opening gating bugs and for any agent asked to verify whether a burn-down is actually complete.
...
- Producer: [`ProjectState::add_evidence`](crates/application/src/state/mod.rs) pushes an `Evidence` record into `ticket_evidence: BTreeMap<String /*ticket id*/, Vec<Evidence>>`. Each `Evidence` holds { label : String, detail : String }.
- Production writer: inside `RunTestUseCase::execute()`, immediately before transitioning to Verified.
...
pub async fn compose_role_system(files,...,role_key) -> String      // BASE + standards + resolved body
pub async fn load_role_override(files,...,role_key) -> Option<String>
pub fn resolve_role_body<'a>(embedded,&Option<&str>) -> Cow<'a str> // pure precedence decision
```
...
CXA-F022 introduces no configuration settings. Burn-down correctness comes from data invariants encoded directly in test predicates and in the production promotion path, not from runtime switches; the only workflow flags that influence whether a bug can reach Verified (e.g. evidence gating on a deployed app, or human gate_verify) predate this ticket and live under WorkflowConfig in crates/application/src/config.rs.
...
- **Empty set is not completion**: `burndown_complete()` returns false when there are no gating bugs at all, so an unrecorded/total absence does not masquerade as done.
- **Prompt composition must stay intact** (AC#5): any prompt-system fix that drops, reorders, or concatenates BASE / ENGINEERING_STANDARDS / role section without blank-line separation breaks every role that calls `system_prompt`.

Deliberately NOT done here: F022 adds no dedicated burn-down orchestrator or endpoint — it enforces the standard through the existing TEST verification path plus gate tests; it does not auto-file or auto-triage new bugs.

## Code map

These paths implement CXA-F022 and its acceptance gate:

- `crates/app/tests/burndown_f022_gate.rs` — acceptance gate: helpers `gating_bugs()`, `recorded_regression_pass()`, `reopened_copy_exists()`, composite predicate `burndown_complete()`; unit tests AC#1–AC#4b and production-path test AC#2/#3 driving `RunTestUseCase`.
- `crates/application/src/use_cases/run_test.rs` — production change: TEST promotes Fixed → Verified and records "REGRESSION TEST" QA evidence (`state.add_evidence`) per CXA-F022 AC#2/#3.
- `crates/application/src/prompts.rs` — F001 surface under AC#5 guard: constants BASE / ENGINEERING_STANDARDS / role bodies, plus regression test module asserting system-prompt composition (`system_prompt_composition_tests`).
- `crates/application/src/prompts_resolve.rs` — role-body override seam exercised by prompts_resolve tests; pure resolution via files port (`WorkspaceFilesPort`).
...
- `crates/application/src/state/mod.rs` — evidence storage model consumed by the predicates: field map ``ticket_evidence`` + producer method add_evidence(…).

## Related

- **F001 prompt system** — the feature these four bugs gate; its surface invariants are pinned by AC#5 here. The override seam it builds on is described in [prompts_resolve].
